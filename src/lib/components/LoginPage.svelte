<script>
	...
	export let roleLabel = 'Back-office municipal';
	export let roleHint  = 'Accès réservé aux agents municipaux autorisés.';
...
<svelte:head>
	<title>{roleLabel} — {$commune?.nom_app || 'DigiCommune'}</title>
</svelte:head>
	...
	<main class="flex-1 flex items-center justify-center px-4 py-12">
		<div class="w-full max-w-sm">
			<div class="text-center mb-8">
				...
				<p class="text-sm text-gray-500">{$commune?.nom || roleLabel}</p>
			</div>
			...
			<p class="text-center text-xs text-gray-400 mt-6">{roleHint}</p>
		</div>
	</main>
